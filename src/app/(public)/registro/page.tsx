...
import { registerSchema, verifyCodeSchema } from '@/validators/authSchemas';

const registerInitialValues = { name: '', email: '', password: '', confirmPassword: '', acceptTerms: false };
const verifyInitialValues = { verificationCode: '' };

export default function RegistroPage() {
  const { register } = useAuth();
  const router = useRouter();
  const [step, setStep] = useState<'form' | 'verify'>('form');
  const [pendingRegistration, setPendingRegistration] = useState<typeof registerInitialValues | null>(null);
  const [generatedCode, setGeneratedCode] = useState('');
  const [submitError, setSubmitError] = useState('');

  const handleRegisterSubmit = (values: typeof registerInitialValues) => {
    setSubmitError('');
  ...
  const handleVerifySubmit = async (
    values: typeof verifyInitialValues,
    { setSubmitting, setStatus }: FormikHelpers<typeof verifyInitialValues>,
  ) => {
    setStatus(null);
    setSubmitError('');
    ...
    <div className="flex min-h-dvh items-center justify-center bg-gray-950 px-4 py-8 -mx-4 -my-8">
      <Card className="w-full max-w-md border-gray-800 bg-gray-900">
        ...
        {step === 'form' ? (
          ...
          <Formik initialValues={verifyInitialValues} validationSchema={verifyCodeSchema} onSubmit={handleVerifySubmit}>
            {({ isSubmitting, status }) => (
              <Form className="mt-6 flex flex-col gap-4">
                <Alert color="info">
                ...
                <div className="flex flex-col gap-2">
                  ...
                  <Field
                    ...
                    inputMode="numeric"
                  />
                  <ErrorMessage name="verificationCode" component="p" className="text-sm text-red-500" />
                </div>

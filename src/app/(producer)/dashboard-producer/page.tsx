...
export default function DashboardProducer() {
  // mock de eventos para simular la lista y selección
  const [eventos, setEventos] = useState<Evento[]>([
    {
      id: "EV-001",
      nombre: "Festival de Rock Neofolk",
      estado: "PROCESO",
      precioBase: 4500,
      capacidad: 500,
      ticketsVendidos: 120,
  ...
  const crearNuevoEvento = () => {
    ...
    // Selecciona automáticamente el nuevo evento y carga su formulario
    setEventoSeleccionado(nuevo);
    setFormNombre(nuevo.nombre);
    setFormPrecio(nuevo.precioBase);
    setFormCapacidad(nuevo.capacidad);
    setSeccionActiva("ajustes");
    setStatusScanner("idle");
  };
    ...
    <div className="p-4 md:p-8 max-w-7xl mx-auto min-h-screen bg-background text-text transition-colors">
      <div className="mb-8 border-b-4 border-text pb-6 flex flex-col md:flex-row justify-between items-start md:items-end gap-4">
        ...
        <div className="bg-surface border-2 border-text px-4 py-2 font-mono text-xs font-bold shadow-[2px_2px_0px_0px_var(--color-text)]">
          PRODUCER_ID:{" "}
          <span className="text-primary dark:text-primary">#BC-84910</span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        ...
        <div className="lg:col-span-5 space-y-4">
          <div className="flex justify-between items-center border-b-2 border-text pb-2">
            ...
            {/* Botón Añadir Evento Neo-Brutalista */}
            <button
              ...
              title="Crear un nuevo espectáculo"
            >
              + Añadir Evento
            </button>
          </div>
        ...
        {/* COLUMNA DERECHA: CONFIGURACIÓN Y SCANNER */}
        <div className="lg:col-span-7 space-y-6">
          {/* botones de selección de sección blindados */}
          ...
          {/* ESTADO 1: NINGÚN EVENTO SELECCIONADO */}
          {!eventoSeleccionado && (
            <div className="border-2 border-dashed border-border p-12 text-center bg-surface/30">
              <p className="font-mono text-sm uppercase text-text-soft font-bold tracking-wide">
                [ Seleccioná un evento de la lista o creá uno nuevo para
                gestionar ]
              </p>
            </div>
          )}

          {/* ESTADO 2: EVENTO SELECCIONADO ACTIVO */}
          {eventoSeleccionado && (
            <div>
              {/* SECCIÓN AJUSTES / RESUMEN */}
              {seccionActiva === "ajustes" && (
                      ...
                      <div className="space-y-4">
                        <div>
                          ...
                          <input
                            ...
                            className="w-full border-2 border-text p-2.5 bg-background font-bold uppercase text-sm focus:outline-none focus:ring-1 focus:ring-primary"
                          />
                        </div>
                    ...
                    <div
                      className={`w-full aspect-video border-2 border-text flex flex-col items-center justify-center p-6 relative overflow-hidden transition-all duration-300 ${
                        statusScanner === "success"
                          ? "bg-success/20 dark:bg-success/10 border-success"
                          : statusScanner === "error"
                            ? "bg-red-600/20 dark:bg-red-500/10 border-red-600 dark:border-red-500"
                            : "bg-black"
                      }`}
                    >
                      {statusScanner === "idle" && (
                        <div className="absolute inset-x-0 h-0.5 bg-primary shadow-[0_0_8px_1px_rgba(204,255,0,0.6)] top-1/2 -translate-y-1/2 animate-bounce"></div>
                      )}
                      {statusScanner === "idle" && (
                      ...
                      {statusScanner === "success" && (
                        <div className="text-center font-mono space-y-1">
                          ...
                          <p className="text-text text-xs font-bold bg-surface-2 border border-text px-3 py-1 mt-2 inline-block font-mono">
